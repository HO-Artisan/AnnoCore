 ...
 * {@code commit(null)} is correct on both Fabric and NeoForge and the call can live in an
 * Architectury {@code :common} subproject.
 * <p>
...
public final class ArchitecturyRegistryBinding<T> implements RegistryBinding<T> {
    ...
    public void register(@NotNull String modId, @NotNull List<RegistryEntry<T>> entries, Object platformContext) {
        Identifier registryId = Identifier.tryParse(kind.id());
        if (registryId == null) {
            throw new IllegalStateException("Not a valid registry id: " + kind.id());
        }
        DeferredRegister<T> deferred = DeferredRegister.create(modId, RegistryKey.<T>ofRegistry(registryId));
